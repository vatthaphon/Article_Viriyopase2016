...
x_lin = unique(x_lin);
% m_xlabel = 'I_{0,E} [\muA/cm^2]';
m_xlabel = '';
y_lin = unique(y_lin);
% m_ylabel = '\sigma_{E} [mV]';
m_ylabel = '';
...
begin_id = sum(sum(x_lin < x_lim));
x_lin = x_lin(1:1:(begin_id - 1));
x_lin = x_lin./nw_A_e.*(1e-12).*(1e+6);
tmp = tmp(:, 1:1:(begin_id - 1));

h = imagesc(x_lin, y_lin, tmp);set(gca,'YDir','normal');set(h,'alphadata',~isnan(tmp)) 
caxis([0.0 60.0]);
% colorbar
% % % colorbar('YTick',[0 10 20 30 40], 'YTickLabel', {'';'';'';'';''})    
set(gca,'XTick',[0 0.1 0.2 0.3],'XTickLabel',{'';'';'';'';''});
...
make_me_pretty(gcf, ...
    ...
    [], 12)
maximize_a_fig(gcf);
m_savefig('IappE_vs_SigmaWNE_Efreq_for_paper3_v2', 'eps');

figure(fig_id);fig_id=fig_id+1;
tmp = reshape(MFR_dt(:, 1), N_y, N_x);
% m_title = 'MFR_E [spks/s]';
m_title = '';
tmp = tmp(:, 1:1:(begin_id - 1));
h = imagesc(x_lin, y_lin, tmp);set(gca,'YDir','normal');set(h,'alphadata',~isnan(tmp)) 
caxis([0.0 4.0]);
...
make_me_pretty(gcf, ...
    ...
    [], 12)
maximize_a_fig(gcf);
m_savefig('IappE_vs_SigmaWNE_MFRE_for_paper3_v2', 'eps');

figure(fig_id);fig_id=fig_id+1;
tmp = reshape(MFR_dt(:, 2), N_y, N_x);
% m_title = 'MFR_I [spks/s]';
m_title = '';
tmp = tmp(:, 1:1:(begin_id - 1));
h = imagesc(x_lin, y_lin, tmp);set(gca,'YDir','normal');set(h,'alphadata',~isnan(tmp)) 
caxis([0.0 60.0]);
...
make_me_pretty(gcf, ...
    ...
    [], 12)
maximize_a_fig(gcf);
m_savefig('IappE_vs_SigmaWNE_MFRI_for_paper3_v2', 'eps');
