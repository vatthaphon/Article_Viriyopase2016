...
{	
$mjob_file = "./mjob_RAP_type2int_reduce.sh";
...
#$begin_Val13		=19000;$end_Val13=$begin_Val13;$len_Val13=1;
...
$FN=$REG_TAG."BW_EEEIIEII_IappEx${Iapp_E}_IappI${Iapp_I}_MFRe${E_firing_rate}_DiracSupraE_rndV${rnd_init_V}_dt${dt}_tEnd${tEnd}_Ni${N_i}_Ne${N_e}";
...
####################### Send commands #######################
	...
	else
	{
		...
		if ($wait2see == 1)
		{
			$cmd="./hh_network_PING_ING_BW_PulseSyn \"$MYFLAG_STR\" ";
			system($cmd);
		}
		else
		{
			$cmd="nohup nice -n 19 ./hh_network_PING_ING_BW_PulseSyn \"$MYFLAG_STR\" >nohup\"$done\".out 2>&1 &";
			system($cmd);
		}
